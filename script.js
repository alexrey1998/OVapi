...
document.addEventListener("DOMContentLoaded", function () {
  // Passage de STOP_NAME de const à let pour pouvoir le modifier
  let STOP_NAME = "Lancy-Bachet, gare";
  
  const stopNameEl = document.getElementById("stop-name");
  stopNameEl.textContent = STOP_NAME;
  
  // Rendre le titre éditable au clic
  stopNameEl.addEventListener("click", function() {
    this.contentEditable = "true";
    this.focus();
  });
  
  // Lorsque l'édition est terminée, mettre à jour STOP_NAME, réinitialiser les filtres et relancer la recherche
  stopNameEl.addEventListener("blur", function() {
    this.contentEditable = "false";
    STOP_NAME = this.textContent.trim();
    // Réinitialiser la sélection des lignes pour le nouvel arrêt
    selectedLines.clear();
    fetchDepartures();
  });
